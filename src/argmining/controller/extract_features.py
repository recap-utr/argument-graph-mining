...
def embedding(span):
    ...
    if config["nlp"]["embeddings"] == "spacy":
        ...
        embed = list(np.mean(embedder.encode(str(span))), axis=1)  # type: ignore
...
def freq(span):
    ...
    sd = [0] * len(vocab)  # type: ignore # noqa: F821
    text = str(span).translate(str.maketrans("", "", string.punctuation))
    tok = [stemmer.stem(token) for token in text.split(" ")]
    for idx, word in enumerate(vocab):  # type: ignore # noqa: F821
        if word in tok:
            sd[idx] = freqdist[word]  # type: ignore # noqa: F821
    return tuple(sd)
