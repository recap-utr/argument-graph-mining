...
from argmining.adu.classify import predict_clpr, predict_mc
...
Mappings = dict[t.Any, t.Optional[MappingEntry]]
...
def run(
    ...
    generated_relations_end2end: dict[str, list[Relation]],
    ...
    statistic.snodes_agreement_end2end = eval_snodes(
        benchmark_graph,
        inode_mappings_end2end,
        generated_relations_end2end,
    )
    statistic.snodes_agreement_preset = eval_snodes(
        benchmark_graph,
        inode_mappings_preset,
        generated_relations_preset,
    )
...
def _strip_snodes(base_graph: ag.Graph) -> ag.Graph:
    g = ag.copy(base_graph, config=ag.load.Config(nlp=nlp.parse))
...
def eval_inodes(
    ...
    return sum(entry.sim for entry in inode_mappings.values() if entry) / max(
        len(generated_graph.atom_nodes), len(benchmark_graph.atom_nodes), 1
    )
...
def eval_snodes(
    ...
    for snode_benchmark in benchmark_graph.scheme_nodes.values():
            ...
            for outgoing_benchmark in outgoing_benchmarks:
                ...
                if incoming_generated and outgoing_generated:
                    total_step, matching_step = _eval_snodes_category(
                        type(snode_benchmark.scheme).__name__
                        if snode_benchmark.scheme
                        else "Unknown",
...
def _eval_snodes_category(
    ...
    incoming_generated: ag.AtomNode,
    outgoing_generated: ag.AtomNode,
    relations: Mapping[str, Iterable[Relation]],
) -> tuple[int, int]:
    total_relations = 0
    matching_relations = 0

    # relations_generated = relations.get(incoming_generated.raw_text, list())
    relations_generated = relations[incoming_generated.plain_text]

    for relation_generated in relations_generated:
        if (
            outgoing_generated
            and outgoing_generated.plain_text == relation_generated.adu
        ):
            total_relations += 1
...
@Language.component("preset_segment")
def preset_segment(doc):
    for token in doc[:-1]:
...
def preset_adus(
...
) -> tuple[Doc, dict[str, list[Relation]], ag.Graph]:
    sents = []
    labels = []
    mc_list = []

    for node_key, node in graph.atom_nodes.items():
        sent_text = node.plain_text
    ...
    parse.add_pipe("preset_segment", before="parser")
    doc = parse(doc_text)
    parse.remove_pipe("preset_segment")

    total_inodes = len(graph.atom_nodes)
...
def _map_inodes(benchmark_graph: ag.Graph, generated_graph: ag.Graph) -> Mappings:
    ...
    benchmark_mappings = _map(
        benchmark_graph.atom_nodes,
        generated_graph.atom_nodes,
        similarity.nodes,
    )
...
def _map_edges(benchmark_graph: ag.Graph, generated_graph: ag.Graph) -> Mappings:
    ...
    for bench_edge in benchmark_graph.edges.values():
        edge_mappings[bench_edge] = None

        for gen_edge in generated_graph.edges.values():
            gen_nodes = (gen_edge.source, gen_edge.target)
            bench_start = inode_mappings[bench_edge.source]
            bench_end = inode_mappings[bench_edge.target]
