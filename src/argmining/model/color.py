...
def _genhex(x):
    ...
    return f"#{_clamp(uints[0]):02x}{_clamp(uints[1]):02x}{_clamp(uints[2]):02x}"
...
def gethsv() -> Generator[str, None, None]:
    return _flatten(map(_genhsv, _getfracs()))  # type: ignore


def getrgb() -> Generator[str, None, None]:
    return map(_genrgb, gethsv())  # type: ignore


def gethtml() -> Generator[str, None, None]:
    return map(_genhtml, getrgb())  # type: ignore


def gethex() -> Generator[str, None, None]:
    return map(_genhex, getrgb())  # type: ignore
