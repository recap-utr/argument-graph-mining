...
def get_model():
    ...
    if model_type == "SVC":
        ...
        model = xgb.XGBRFClassifier()  # type: ignore
        ...
        if train_method == "GridSearch":
            ...
            estimator_dict["xgbrf"] = GridSearchCV(
                xgb.XGBRFClassifier(),  # type: ignore
        ...
        for i, m in enumerate(stacks):
            if isinstance(m, list):
                ...
                layer = StackingClassifier(  # type: ignore
                    estimators=sublayer,
        ...
        if len(single_layer) > 0:
            model = StackingClassifier(  # type: ignore
...
def print_metrics(targets, predictions):
    ...
    print(f"Accuracy\t{accuracy_score(targets, predictions):8.8f}")
    print(f"Precision:\t{precision_score(targets, predictions):8.8f}")
    print(f"Recall:\t{recall_score(targets, predictions):8.8f}")
    print(f"F1-Score:\t{f1_score(targets, predictions):8.8f}")
    return (
