...
def single_run(input_files, label_files, test_files, test_labels, adu_mode="true"):
    ...
    if adu_mode == "true":
        ...
        clpr_feats = []
        for idx, l in enumerate(t_doc._.Labels):  # noqa: E741
            if l == 1:
        ...
        cl_iter = 0
        correct_count = 0
        for idx, _l in enumerate(orig_adus_labels):
        ...
        print(f"Precision:\tCLPR\t{prec:8.8f}")
...
def LOOCV(input_files, label_files, test_files, test_labels, n_runs=5, adu_mode="true"):
    ...
    for i in range(n_runs):
        print(i)
        input_files, label_files = shuffle(input_files, label_files)  # type: ignore
        ...
        if adu_mode == "true":
            ...
            avg_cl_acc += prec  # type: ignore
    avg_cl_acc *= 1 / n_runs
    print(f"Avg Accuracy:\tCLPR\t{avg_cl_acc:8.8f}")
    if adu_mode == "true":
        avg_cl_prec *= 1 / n_runs
        avg_cl_rec *= 1 / n_runs
        avg_cl_f1 *= 1 / n_runs
        print(f"Avg Precision:\tCLPR\t{avg_cl_prec:8.8f}")
        print(f"Avg Recall:\tCLPR\t{avg_cl_rec:8.8f}")
        print(f"Avg F1-Score:\tCLPR\t{avg_cl_f1:8.8f}")
